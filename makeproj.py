...
def get_c_config(name: str) -> Dict:
    ...
    if click.prompt("Do you want to add your libft?", type=bool):
        config['libft_path'] = os.path.abspath(os.path.expanduser(
                click.prompt('Please enter the path to your libft',
...
def get_py_config(name: str) -> Dict:
    ...
    if click.prompt("Do you want to create testing files?", type=bool):
        config['testing'] = True
        if click.prompt("Do you want to install pyTest testing framework?", type=bool):
            config['pytest'] = True
    if click.prompt("Create a author file?", type=bool):
        config['author'] = click.prompt("Input your username", type=str)
    if click.prompt("Create a README.md?", type=bool):
...
def create_program(name: str, lang: str, output_dir: str, config: Dict) -> None:
    ...
    if lang == 'c':
        ...
        if 'libft_path' in config:
            copytree(config['libft_path'], os.path.join(output_dir, 'libft'))
    elif lang == 'py':
        ...
        if 'depend' in config:
            copy(config['depend'], os.path.join(output_dir, 'requirements.txt'))
            if 'venv' in config:
        ...
        if 'testing' in config:
            copy(os.path.join(PYTHON_TEMPLATE_PATH, 'test_project.py'), os.path.join(output_dir, 'test_project.py'))
            if 'pytest' in config:
                if 'venv' in config:
                    subprocess.run(["venv/bin/pip", "install",
